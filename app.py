...
with col1:
    ...
    top0color = st.select_slider("색상 톤", options=["밝음", "무난함", "어두움"])
    with col2:
        st.subheader("하의")
...
with tab1:
    ...
    shoes = sts.selectbox("신발 선택", ["스니커즈", "운동화", "구두", "슬리퍼"])
    with st.expander("신발 선택 팁보기"):
        st.info("너무 튀는 신발은 지양하도록 해요!")
        with tab2:
            st.wrie("포인트 아이템:")
            acc = st.multiselect("엑세서리 추가", ["모자", "안경", "목걸이", "가방"])
